-- =====================================================================
-- 2D.3 (pré-condição) — CADA LINHA `pr` ASSINADA TEM QUE VIRAR UMA LINHA
-- DE PAGAMENTO PERSISTIDA
--
-- A invariante, na formulação do usuário:
--
--     Um Romaneio de Retorno só pode ser selado se cada linha `pr`
--     assinada corresponder a exatamente uma linha de pagamento
--     realizado persistida pela mesma transação.
--
-- "O id não pode colidir com o do previsto" é CONSEQUÊNCIA disso, não a
-- regra.
--
-- ---------------------------------------------------------------------
-- O DEFEITO QUE ELA FECHA, e ele é silencioso
--
-- O `pagamento_id` do pagamento PREVISTO é o mesmo uuid da entrega. É
-- desenho, não acidente: `criarPagamentoPrevisto` usa id determinístico
-- porque a relação é 1:1, e é isso que dá idempotência ao reenvio da
-- fila sem precisar de upsert.
--
-- A tela do retorno vai querer pré-preencher o realizado com o previsto,
-- pra o caixa só confirmar. Copiando o objeto inteiro, o `pagamento_id`
-- vai junto:
--
--     DCRR1 diz    pr E1 <id-do-previsto> dinheiro 12390 0
--     insert bate em `on conflict (id) do nothing`
--     nada é gravado, nada é levantado
--     romaneio SELADO afirmando um realizado que não existe
--
-- Achado em 2026-08-20 lendo o vale que a conferência da 2D.2 imprime.
-- Não apareceu em revisão de código nem em teste.
--
-- ---------------------------------------------------------------------
-- POR QUE NO BANCO, SE A TELA JÁ VAI RECUSAR
--
-- Porque segurança não pode depender da UI. Um cliente antigo, com
-- defeito ou manipulado manda o mesmo payload — e o documento chega aqui
-- perfeitamente assinado. O guard da tela existe pra ninguém chegar até
-- cartão, PIN e duas assinaturas pra só então descobrir; este existe pra
-- o documento inconsistente não existir de jeito nenhum.
--
-- Mesma divisão da 2C.2: o cliente evita o custo, o banco impede o dano.
--
-- ---------------------------------------------------------------------
-- POR QUE EXCEÇÃO, E NÃO CONFLITO REGISTRADO
--
-- A 2B decidiu que recusa vira `romaneios` com `status = 'conflito'`,
-- porque quando o retorno chega aqui as duas partes já assinaram e a
-- prova não pode sumir num rollback. Isso continua valendo para todas as
-- recusas de lá.
--
-- Esta é diferente: o documento é INTERNAMENTE INCONSISTENTE. Ele afirma
-- um pagamento que a própria transação não conseguiu gravar. Preservá-lo
-- como conflito criaria uma evidência de algo que não pode ter
-- acontecido, e que alguém poderia tentar reaproveitar depois.
--
-- ---------------------------------------------------------------------
-- O QUE MUDA, E O QUE NÃO MUDA
--
-- Esta é a QUARTA definição de `selar_romaneio_retorno_interno`
-- (20260820130000, 20260820160000, e agora esta). Ela foi obtida
-- PATCHEANDO a mais recente por script, e o diff foi conferido antes de
-- virar migration:
--
--     14 linhas de código acrescentadas, 0 removidas
--     as 4 linhas de `digest(...)` byte a byte IDÊNTICAS
--
-- A fórmula do hash não muda. O `on conflict (id) do nothing` continua
-- lá — ele é o que impede reenvio de duplicar pagamento, e tirá-lo
-- trocaria um defeito silencioso por outro.
--
-- Conferência: `scripts/conferir-2d3-no-sql-editor.sql`.
-- =====================================================================
